-- create AEOS object store database, you could update FILENAME as your requirement.
-- Please make sure you change the drive and path to your MSSQL database.
CREATE DATABASE AEOS
ON PRIMARY
(  NAME = AEOS_DATA,
   FILENAME = 'C:\MSSQL_DATABASE\AEOS_DATA.mdf',
   SIZE = 400MB,
   FILEGROWTH = 128MB ),

FILEGROUP AEOSSA_DATA_FG
(  NAME = AEOSSA_DATA,
   FILENAME = 'C:\MSSQL_DATABASE\AEOSSA_DATA.ndf',
   SIZE = 300MB,
   FILEGROWTH = 128MB),

FILEGROUP AEOSSA_IDX_FG
(  NAME = AEOSSA_IDX,
   FILENAME = 'C:\MSSQL_DATABASE\AEOSSA_IDX.ndf',
   SIZE = 300MB,
   FILEGROWTH = 128MB)

LOG ON
(  NAME = 'AEOS_LOG',
   FILENAME = 'C:\MSSQL_DATABASE\AEOS_LOG.ldf',
   SIZE = 160MB,
   FILEGROWTH = 50MB )
GO

ALTER DATABASE AEOS SET RECOVERY SIMPLE
GO

ALTER DATABASE AEOS SET AUTO_CREATE_STATISTICS ON
GO

ALTER DATABASE AEOS SET AUTO_UPDATE_STATISTICS ON
GO

ALTER DATABASE AEOS SET READ_COMMITTED_SNAPSHOT ON
GO
